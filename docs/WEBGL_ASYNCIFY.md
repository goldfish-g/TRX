...
| `Shell_InitIDBFS()`           | Awaits frontend persistence mount + sync  |
| `Shell_WaitForUserInput()`    | Awaits frontend start gate                |
| `Shell_LoadModGameData()`     | Awaits frontend mod-data staging          |
| `Shell_ShowProfileSelector()` | Awaits frontend profile selection (Exit)  |
| `FMV_Play()` (emscripten)     | Video frame sync loop                     |

Adapter-side calls use a flag-polling pattern: an `EM_JS` stub kicks
the Promise returned by `Module.trxAdapters.<method>()` and parks a
`done` flag on `Module._trxState`; the C side spins in a `Clock_Delay`
loop until the flag flips. This piggybacks on the already-whitelisted
Asyncify yield rather than relying on `EM_ASYNC_JS`'s rewind machinery,
which is fragile in the presence of our function-pointer-heavy
game-flow dispatch. See
...
through function pointers. The adapter-awaiting entry points
(`Shell_InitIDBFS`, `Shell_WaitForUserInput`, `Shell_LoadModGameData`,
`Shell_ShowProfileSelector`) are listed explicitly — they spin in a
`Clock_Delay` loop until their adapter Promise resolves, and that
`Clock_Delay` is what actually yields through Asyncify.
