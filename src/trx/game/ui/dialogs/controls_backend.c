...
static const M_OPTION m_Options[] = {
    ...
    { .gs_id = GS_ID("general/settings/controls/backend/touch"),
      .backend = INPUT_BACKEND_NUMBER_OF,
      .requires_touch = true },
    { .gs_id = nullptr },
};
...
void UI_ControlsBackend_Init(UI_CONTROLS_BACKEND_STATE *const s)
{
    ...
    for (int32_t i = 0; m_Options[i].gs_id != nullptr; i++) {
        if (m_Options[i].requires_touch && !g_Config.input.enable_touch_controls) {
            continue;
        }
        if (m_Options[i].backend == (int32_t)g_Config.input.backend) {
            sel_row = count;
        }
