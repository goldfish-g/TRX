...
{
    M_PRIV *const p = &m_Priv;
    const float ri = circle.r_inner;
    const float ro = circle.r_outer;
    if (ro <= ri) {
        return;
    }

    const RGBA_F col = circle.color;
    ...
    if (ri > 0.0f) {
        M_StageRingBand(
            p, circle.cx, circle.cy, ri, ri + fw, circle.z, col_0, col);
    }
    ...
    if (r_body_inner < r_body_outer) {
        M_StageRingBand(
            p, circle.cx, circle.cy, r_body_inner, r_body_outer, circle.z, col,
            col);
    }

    // Outer feather: ro-fw → ro, solid→transparent
    M_StageRingBand(p, circle.cx, circle.cy, ro - fw, ro, circle.z, col, col_0);
}
